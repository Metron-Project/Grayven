...
TEST_URL: Final[str] = f"https://www.comics.org/api{TEST_ENDPOINT}"
SUCCESS_JSON: Final[dict] = {"results": []}
FAILURE_JSON: Final[dict] = {"detail": "Rate Limited"}
...
def add_200_response(mock: HTTPXMock) -> None:
    mock.add_response(url=TEST_URL, status_code=200, json=SUCCESS_JSON, is_reusable=True)


def add_429_response(mock: HTTPXMock) -> None:
    mock.add_response(url=TEST_URL, status_code=429, json=FAILURE_JSON, is_reusable=True)
...
def make_session(limiter: Limiter) -> GrandComicsDatabase:
    return GrandComicsDatabase(
        email="UNSET",
        password="UNSET",  # noqa: S106
        cache=SQLiteCache(path=Path("tests/cache.sqlite")),
        limiter=limiter,
    )


@pytest.fixture
def second_session() -> GrandComicsDatabase:
    return make_session(limiter=make_limiter(Rate(2, Duration.SECOND)))


@pytest.fixture
def day_session() -> GrandComicsDatabase:
    return make_session(limiter=make_limiter(Rate(5, Duration.DAY)))


@pytest.fixture
def dual_session() -> GrandComicsDatabase:
...
class TestSecondLimit:
    def test_within_limit_not_delayed(
        ...
        second_session: GrandComicsDatabase,
    ) -> None:
...
class TestDayLimit:
    def test_within_limit_not_delayed(
        ...
        freezer: FrozenDateTimeFactory,
        httpx_mock: HTTPXMock,
        day_session: GrandComicsDatabase,
    ) -> None:
        add_200_response(mock=httpx_mock)
...
class TestDualLimit:
    def test_second_limit_still_applies(
        ...
        freezer: FrozenDateTimeFactory,
        httpx_mock: HTTPXMock,
        dual_session: GrandComicsDatabase,
    ) -> None:
        add_200_response(mock=httpx_mock)
...
class TestRateLimitError:
    def test_server_429_raises_rate_limit_error(
        self, httpx_mock: HTTPXMock, second_session: GrandComicsDatabase
    ) -> None:
    ...
    def test_server_429_raised_afyer_ratelimiting(
        self, httpx_mock: HTTPXMock, second_session: GrandComicsDatabase
    ) -> None:
        httpx_mock.add_response(url=TEST_URL, status_code=200, json=SUCCESS_JSON)
        httpx_mock.add_response(url=TEST_URL, status_code=200, json=SUCCESS_JSON)
        httpx_mock.add_response(url=TEST_URL, status_code=429, json=FAILURE_JSON)
